...
1. Theoretical five-angle workspace grid (offline, no movement)
   It evaluates all 5^6 = 15,625 joint combinations. Each joint uses five
   evenly spaced values across its own manufacturer limits.

2. Physical five-angle single-joint sweeps (30 movements)
   It moves one joint through five values from that joint's own limits while the other five remain at a
...
def five_values_for_joint(joint_number: int, common_angles: Optional[List[float]] = None) -> List[float]:
    """Return the five test values for one joint.

    When ``--angles`` is not supplied, the values are evenly spaced across that
    joint's own manufacturer limit. For example, Joint 1 becomes:
    [-170, -85, 0, 85, 170].

    Supplying ``--angles`` overrides this and uses the same five values for all
    selected joints.
    """
    if common_angles is not None:
        return [float(v) for v in common_angles]
    lower, upper = JOINT_LIMITS_DEG[joint_number]
    return np.linspace(lower, upper, 5).tolist()


def all_joint_test_values(common_angles: Optional[List[float]] = None) -> List[List[float]]:
    """Build the six separate five-angle lists used by the FK workspace grid."""
    return [five_values_for_joint(j, common_angles) for j in range(1, 7)]


def generate_theoretical_grid(fk: UrdfFK, joint_angle_sets: List[List[float]], output_dir: str) -> str:
    """Evaluate 5^6 = 15,625 FK points using each joint's own angle range.
...
def physical_sweep(robot: MyCobot280, fk: UrdfFK, args, output_dir: str) -> str:
    ...
    with open(path, "w", newline="") as file:
        ...
        for joint_number in args.joints:
            ...
            sweep_angles = five_values_for_joint(joint_number, args.angles)
            print("  Test angles:", [round(v, 3) for v in sweep_angles])
...
def parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--angles", type=float, nargs=5, default=None,
                   help="optional common five-angle override; otherwise each joint uses five values across its own limits")
...
def main() -> int:
    ...
    joint_angle_sets = all_joint_test_values(args.angles)
    print("Five test angles used for each joint:")
    for joint_number, values in enumerate(joint_angle_sets, start=1):
        print(f"  J{joint_number}: {[round(v, 3) for v in values]}")

    if not args.skip_theoretical_grid:
        print("Generating 15,625-point theoretical five-angle FK grid...")
        print("Saved:", generate_theoretical_grid(fk, joint_angle_sets, output_dir))
    ...
    print("WARNING: the physical robot will now move through values that may include full joint limits.")
    print("Full joint limits can cause self-collision or collision with the table, camera, gripper, or surroundings.")
